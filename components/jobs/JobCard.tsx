...
const JobCard: React.FC<JobCardProps> = ({
  ...
  const getCardBorderColor = (status: JobStatus) => {
    const colors = {
      [JobStatus.APPLIED]:
        'border-blue-300 bg-blue-50/30 dark:border-blue-600 dark:bg-blue-900/20',
      [JobStatus.TEST_PENDING]:
        'border-yellow-300 bg-yellow-50/30 dark:border-yellow-600 dark:bg-yellow-900/20',
      [JobStatus.TEST_COMPLETED]:
        'border-orange-300 bg-orange-50/30 dark:border-orange-600 dark:bg-orange-900/20',
      [JobStatus.INTERVIEW]:
        'border-purple-300 bg-purple-50/30 dark:border-purple-600 dark:bg-purple-900/20',
      [JobStatus.ACCEPTED]:
        'border-green-300 bg-green-50/30 dark:border-green-600 dark:bg-green-900/20',
      [JobStatus.REJECTED]:
        'border-red-300 bg-red-50/30 dark:border-red-600 dark:bg-red-900/20',
    };
    ...
    >
      ...
      >
        <div className='flex justify-between items-start mb-2'>
          <h3
            className={`${isExpanded ? 'text-lg' : 'text-sm'} font-semibold text-[color:var(--color-card-foreground)] ${isExpanded ? '' : 'line-clamp-1'}`}
          >
            {job.title}
          </h3>
        ...
        <p
          className={`${isExpanded ? 'text-base' : 'text-sm'} text-[color:var(--color-primary)] font-medium ${isExpanded ? '' : 'line-clamp-1'} mb-2`}
        >
          {job.company}
        </p>
        ...
        {job.location && (
          <p
            className={`${isExpanded ? 'text-sm' : 'text-xs'} text-[color:var(--color-muted-foreground)] mb-2 ${isExpanded ? '' : 'line-clamp-1'}`}
          >
            📍 {job.location}
          </p>
        )}
        ...
        <div className='flex justify-between items-center'>
          <div className='flex items-center'>
            <span
              className={`px-3 py-1.5 rounded-full text-xs font-medium ${getStatusColor(job.status)}`}
            >
        ...
        <div className='px-6 pb-6 border-t border-[color:var(--color-border)] bg-[color:var(--color-secondary)]/10'>
          <div className='grid grid-cols-1 lg:grid-cols-2 gap-6 mt-4'>
            ...
            <div className='space-y-4'>
              <div>
                <h4 className='text-sm font-semibold text-[color:var(--color-primary)] mb-2'>
                  Informações Básicas
                </h4>
                  ...
                  {job.status === JobStatus.REJECTED ? (
                    <div className='bg-red-50 border border-red-200 p-3 rounded-md dark:bg-red-900/30 dark:border-red-700'>
                      <p className='text-sm text-red-800 dark:text-red-200'>
                        📝 Esta vaga foi rejeitada e está apenas no histórico.
                    ...
                    <div>
                      ...
                      >
                        <option value={JobStatus.APPLIED}>
                          Candidatura Enviada
                        </option>
                        <option value={JobStatus.TEST_PENDING}>
                          Teste Pendente
                        </option>
                        <option value={JobStatus.TEST_COMPLETED}>
                          Teste Concluído
                        </option>
                        <option value={JobStatus.INTERVIEW}>
                          Em Entrevista
                        </option>
