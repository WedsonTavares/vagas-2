...
export const getStatusCssClass = (status: string): string => {
  return (
    STATUS_CSS_CLASSES[status as JobStatus] ||
    STATUS_CSS_CLASSES[JobStatus.APPLIED]
  );
};
...
export const prepareChartData = (byStatus?: Record<string, number>) => {
  // Segurança: se byStatus for undefined ou null, usamos objeto vazio como fallback
  const safeByStatus = byStatus ?? {};
  ...
  return statusOrder
    .map(status => ({
      name: getStatusLabel(status).split(' ')[0], // "Candidatura" em vez de "Candidatura Enviada"
      fullName: getStatusLabel(status), // Nome completo para tooltips
      value: safeByStatus[status] ?? 0, // Quantidade de vagas (fallback seguro)
      color: getStatusColor(status), // Cor do segmento no gráfico
