...
const StatusCards: React.FC<StatusCardsProps> = ({ stats }) => {
   ...
   */
  const navigateToJobsWithFilter = useCallback((status?: string) => {
    const url = status ? `/dashboard/jobs?status=${status}` : '/dashboard/jobs'
    router.push(url)
  }, [router])
  ...
  const cardElements = useMemo(() => {
    return CARD_CONFIGS.map(({ status, label, icon, hoverColor }) => {
      const count = stats.byStatus[status] || 0  // Fallback para 0 se não existir
