...
export async function POST(request: NextRequest) {
  ...
  const exam = examRes.data as { materia: string; notes?: string | null };
  const materiaName = exam.materia;
  ...
  const materias = JSON.parse(fileRaw) as Array<Record<string, any>>;
      ...
      let updated = false;
      if (idx !== -1) {
        const target = materias[idx] as Record<string, any>;
